...
h=1080
filename="./vid1.h265"
command = "nvarguscamerasrc sensor-id=0  timeout={} ! 'video/x-raw(memory:NVMM), width={}, height={}, format=NV12, framerate=30/1' ! nvvidconv flip-method=2 ! 'video/x-raw(memory:NVMM), format=I420' ! omxh265enc peak-bitrate={} ! 'video/x-h265, stream-format=byte-stream' ! h265parse ! qtmux ! filesink location={} -e".format( ctx_vid_duration,w,h,ctx_vid_bitrate,filename) 

print("gst command: {}".format(command) )
...
def capture_context_video():
    ...
    ts = datetime.now()
    filename = save_path + 'ctx_'+ ts.strftime("%Y-%m-%d_%H-%M-%S.%f")+ '' + '.h265'
    ...
    '''
    command = ("nvarguscamerasrc  sensor-id=0  timeout={} ! "
               "'video/x-raw(memory:NVMM), width={}, height={}, format=NV12, framerate=30/1' ! "
               "nvvidconv flip-method=2 ! 'video/x-raw(memory:NVMM), format=I420' ! "
               "omxh265enc peak-bitrate={} ! 'video/x-h265, stream-format=byte-stream' ! h265parse ! qtmux ! "
               "filesink location=./vid1.h265 -e").format(30, 1920, 1080, 8000000)
    '''

    command = ("-e nvarguscamerasrc ! 'video/x-raw(memory:NVMM), width=(int)1920, height=(int)1080, format=(string)NV12, framerate=(fraction)30/1' \
...
! h264parse ! qtmux ! filesink location=./vid2.h264 -e").format(30,1920,1080,8000000)
